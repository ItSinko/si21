...
if( isset( $_REQUEST['aksi'] )){
		...
		switch($aksi){
			case 'baru':
				include 'akuntingoff/add.php';
				break;
				
			case 'edit':
				include 'akuntingoff/change.php';
				break;
				
			case 'delete':
				include 'akuntingoff/hapus.php';
				break;
			case 'history':
				include 'akuntingoff/history_bayar.php';
				break;
...
?>
	...
	<p class="h5">Tabel Piutang(Offline)</p>
	<hr class="my-2">
					...
					<div class="jplist-panel box panel-top">						
					  ...
					  <div class="jplist-panel box panel-top">				
  <button class="btn btn-primary"   onclick="location.href='./index.php?hlm=accoff&aksi=baru'">Tambah Data</button>						
						</div>	
 ...
 <!-- data -->
                    ...
                    <div class="horizon">
                        <table class="demo-tbl">
                            ...
                            <thead class="jplist-panel">
                                ...
                                <tr data-control-type="sort-buttons-group"
									...
									<th width="3%">E-Faktur</th>
									<th width="2%">No Faktur Penjualan</th>
									<th width="6%">
						...
						echo'
                            ...
                            <tbody>
                                ...
                                <tr class=tbl-item>							
									<td >'.$a++.'</td>
									<td class=nolkpp>OFF-'.$row['idorder_off'].'</td>	
									...
									<td>'.number_format($row['jumlah_off']).' '.$row['satuan_prod'].'</td>  
									<td>'.number_format($row['total']).'</td>
									<td>'.$row['diskonacc_off'].' %</td>
									...
									<td>'. $bayarbatas.'</td>
									<td>'.number_format($row['total_lunas']).'<br><a href="?hlm=accoff&aksi=history&idorderacc_off='.$row["idorderacc_off"].'" target="_blank">History Pelunasan</a></td>
									<td>'.number_format($row['selisihharga']).'</td>
									<td>'.$row['kas_off'].'</td>
									<td>'.$row['ketaccoff'].'</td>';  ?>
									<td>
									<a href="?hlm=accoff&aksi=edit&idorderacc_off=<?=$row["idorderacc_off"]; ?>" class="cus-application_form_edit"></a> |
									<a href="#" type="button" class="cus-cross" data-toggle="modal" data-target="#myModal<?=$row["idorderacc_off"]; ?>"></a>
									</td>			
								</tr>
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form role="form" action="akuntingoff\hapus.php" method="get">

                        <?php
                        $idorderacc_off = $row["idorderacc_off"]; 
                        $query_edit = mysqli_query($con,"SELECT * FROM acc_off WHERE idorderacc_off='$idorderacc_off'");
                        //$result = mysqli_query($conn, $query);
                        while ($rows = mysqli_fetch_array($query_edit)) {  
                        ?>

                        <input type="hidden" name="idorderacc_off" value="<?php echo $rows["idorderacc_off"] ?>" >
                       <input type="hidden" name="username" value="<?php echo $_SESSION['username'] ?>" >

                      <div class="form-group">
						<label for="message-text" class="col-form-label">Alasan ID <b>OFF-<?php echo $rows["idorderacc_off"] ?></b> dihapus ?</label>
