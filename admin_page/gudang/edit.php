   ...
   <div class="form-group row">
    ...
    <div class="col-sm-8">
     ...
     <input class="form-control jumlah"  type="text"  style="height:28px; width:70px;" onkeyup="rp();count();"  id="jumlah"placeholder="99"   value= "<?= $gdg_on["jumlah_on"]; ?>" readonly>
		 <input  value="<?= $gdg_on["satuan_prod"]; ?>" class="input-group-text " readonly style="height:28px; width:60px;">
     </div>
     </div>
     </div>
	 </div> 
     ...
     <input type="text" class="form-control harga"  id="nosj" placeholder="123" name="nosj"   value= "<?= $gdg_on["nosj_on"]; ?>" >
     </div>
     </div>
 ...
 <a href="./index.php?hlm=gudangon" formnovalidate  onclick="return confirm('Yakin Batal ?');"  style="position: absolute; right:45%;"  class="btn btn-danger">Batal</a>
	...
	if(nolkpp == "" || nosj == ""|| tglsj == ""|| ket_log == "") {
		...
		$.ajax({
			...
			data: "nolkpp="+nolkpp+"&nosj="+nosj+"&tglsj="+tglsj+"&pabrik_on="+pabrik_on+"&idprod_on="+idprod_on+"&ket="+ket+"&ket_log="+ket_log+"&username="+username,
