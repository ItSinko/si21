...
<div class="container">
  <div class="row">
    ...
    <div class="col-sm">
	 ...
	 <br>
	 Masukan nomer PO kiriman di kolom bawah ini*
	 
	 
	  <div class="form-group row">
     <div class="col-sm-12">
     <input class="form-control" rows="4" placeholder="No PO" id="nopo" name="nopo" required>
     </div>
     </div>
...
if (isset($_POST['lacak'])) {
  ...
  if ($database == 'online') {
	  ...
	  $result=mysqli_query($con,"SELECT * FROM spa_on 
												...
												INNER JOIN  produk_master ON spa_on.idprod_on = produk_master.id_prod WHERE nopo_on ='$nopo'
	 ...
	 echo' 
	  Hasil pencarian dengan nomer PO : '.$nopo.'
	  <table class="table">
  <thead>
    <tr>
      <th scope="col">No</th>
  ...
  <tbody>
	...
	//Status QC
		$qc = $row['nolkppqc_on'];
									 ...
									 if (!empty($qc)) {
									echo '<td  class=status_po><i class="cus-accept" title=">Sudah di QC"></i><br>Sudah di QC</td>';
									 }
									 
									 else {
										echo '<td class=status_po><i class="cus-cancel"></i title="Belum Proses QC"><br>Belum Proses QC</td>';
	...
	//Status ACC
		$acc = $row['nolkppacc_on'];
									 
									 
									 if (!empty($acc)) {
									echo '<td  class=status_po><i class="cus-accept" title="Sudah Terkirim"></i><br>Sudah Dilunasi<br>'.$row['nofakju_on'].'<br>'.$row['tgllunas_on'].'</td>';
									 }
									 
									 else {
										echo '<td class=status_po><i class="cus-cancel"></i title=">Belum Terkirim"><br>Belum Dilunasi</td>';
...
echo'
 <tbody>
	...
	//Status ACC
		$acc = $row['idorderacc_off'];
									 
									 
									 if (!empty($acc)) {
									echo '<td  class=status_po><i class="cus-accept" title="Sudah Terkirim"></i><br>Sudah Dilunasi<br>'.$row['nofakju_off'].'<br>'.$row['tgllunas_off'].'</td>';
									 }
    ...
    <tr>
      ...
      ';	
		...
		$acc = $row['noacc_spb'];
									 ...
									 if (!empty($acc)) {
									echo '<td  class=status_po><i class="cus-accept" title="Sudah Terkirim"></i><br>Sudah Dilunasi<br>'.$row['nofakju_spb'].'<br>'.$row['tgllunas_spb'].'</td>';
									 }
