<html>
<head>
<title>Laporan Keuangan (Offline)</title>
</head>
<style>
...
<body>
<h3 style="margin-bottom: -20px;">Laporan Keuangan (Offline)</h3>					
...
<button class="btn btn-primary" type="export" id="tombolExport"><i class="cus-arrow_down"></i>Export</button>
	    ...
	    <label class="nmprd-fltr">Nama Produk :</label>  
                ...
                $query = mysqli_query($con,"SELECT nam_prod FROM acc_off										
												...
												INNER JOIN piutang_off ON piutang_off.idorderuji_off = acc_off.idorderacc_off
		      ...
		      <label class="aks-fltr">Pemesan :</label>  
                ...
                $query = mysqli_query($con,"SELECT pemesan_off FROM acc_off										
											 ...
											 GROUP by pemesan_off");
                while ($row = mysqli_fetch_array($query)) {
                ?>
                    <option value="<?php echo $row['pemesan_off']; ?>"></option>
                <?php
                }
                ?>
                ...
                $query = mysqli_query($con,"SELECT pabrik FROM acc_off										
...
$result=mysqli_query($con,"SELECT   * ,harga_off * jumlah_off as total
											...
											,IF(iddsb='SPA',
									  ((((harga_off * jumlah_off) - ((harga_off * jumlah_off) * diskonacc_off / 100)) + SUM(nilai))/1.1) * 0.985,
									  (harga_off * jumlah_off) - ((harga_off * jumlah_off) * diskonacc_off / 100) + SUM(nilai) ) AS nilaiterima
									  
									  
									   ,IF(iddsb='SPA',
									  (((((harga_off * jumlah_off) - ((harga_off * jumlah_off) * diskonacc_off / 100)) + SUM(nilai))/1.1) * 0.985)  - realisasiaccoff,
									  ((harga_off * jumlah_off) - ((harga_off * jumlah_off) * diskonacc_off / 100) + SUM(nilai)) - realisasiaccoff ) AS selisihharga
									  
									  ,SUM(nilai) as kirim
									  
									  ,((harga_off * jumlah_off) - ((harga_off * jumlah_off) * diskonacc_off / 100)) + SUM(nilai)  as totpiu
		...
		<td>'. $bayarbatas.'</td>
		<td>'. number_format($row["realisasiaccoff"]).'</td>
