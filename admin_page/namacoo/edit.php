...
<legend class="legend_form">Data Produk</legend>
   ...
   <div class="form-group row">
	...
	<div class="form-group row">
    <label  class="col-sm-3 col-form-label">Nama COO *</label>
    <div class="col-sm-2">
    <input type="text" class="form-control"  placeholder="Nama COO"  id="namacoo" value="<?php echo $prd['namacoo']; ?>">
    </div>
    </div>
...
<button type="submit" class="btn btn-success" style="position: absolute; right:55%;"  onclick="return confirm('Apakah data sudah benar ?');" id="tambah" name="tambah" >Update Data</button>	
 ...
 <a href="./index.php?hlm=namcoo" formnovalidate  onclick="return confirm('Kembali ?');"  style="position: absolute; right:45%;"  class="btn btn-danger">Batal</a>
 </div>
...
$("#check").submit(function(e) {
	...
	if(id_prod == "" || type == ""|| nmprd == ""|| merk == ""|| kategori == "" ||  ket_log  == "") {
		...
		$.ajax({
			type: "POST",
			url: "check_form/edit/namacoo.php",
			data: "id_prod="+id_prod+"&type="+type+"&nmprd="+nmprd+"&merk="+merk+"&kategori="+kategori+"&namacoo="+namacoo+"&ket="+ket+"&ket_log="+ket_log+"&username="+username,
