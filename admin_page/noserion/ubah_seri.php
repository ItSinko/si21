...
$data = mysqli_query($con,"SELECT * FROM seri_on WHERE idseri_on='$idseri_on' ");
...
<legend class="legend_form">Nomer Seri</legend>
     ...
     <div class="col-sm-2">
     <input class="form-control" rows="2" placeholder="ID" id="lkppfk"  value= "<?= $noseri["lkppfk_on"]; ?>"  readonly >
     </div>
     </div>
  ...
  <div class="form-group row">
     ...
     <div class="col-sm-3">
	 <input class="form-control" rows="2" placeholder="Nomer Seri" id="noseri"   value= "<?= $noseri["noseri_on"]; ?>"  >
   ...
   <input type="hidden" value= "<?= $noseri["idseri_on"]; ?>" id="idseri">
...
<a href="./index.php?hlm=noserion&aksi=more&nolkppadm_on=<?php echo $nolkppgdg_on; ?>" formnovalidate  onclick="return confirm('Yakin Batal ?');"  style="position: absolute; right:45%;"  class="btn btn-danger">Batal</a></div>
</form>	
...
$("#check").submit(function(e) {
	...
	if(noseri == "" || ket_log == "") {
		...
		$.ajax({
			type: "POST",
			url: "check_form/edit/noseri.php",
			data: "idseri="+idseri+"&noseri="+noseri+"&lkppfk="+lkppfk+"&ket_log="+ket_log+"&username="+username,
