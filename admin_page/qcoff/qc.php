...
if( isset( $_REQUEST['aksi'] )){
		...
		switch($aksi){
			case 'baru':
				include 'qcoff/newtambah.php';
				break;
			case 'edit':
				include 'qcoff/edit.php';
				break;
			case 'delete':
				include 'qcoff/hapus.php';
				break;
	...
	<hr class="my-2">
					...
					<div class="jplist-panel box panel-top">						
                        ...
                        <div 
						   ...
						   <button class="btn btn-primary"   onclick="location.href='./index.php?hlm=qcoff&aksi=baru'">Tambah Data</button>
					</div>		
 <!-- data --> 
                    <div class="horizon_disable">
                        <table class="demo-tbl">
							...
							<?php 
$a= 1;
$result=mysqli_query($con, "SELECT * FROM qc_off INNER JOIN admjual_off ON qc_off.idorderqc_off = admjual_off.idorderadm_off 
											   ...
											   INNER JOIN produk_master ON produk_master.id_prod =  qc_off.idprodqc_off
											   ORDER BY idorderqc_off + 0 DESC");		
		...
		echo'                 
                            <tbody>                      
                                <tr class=tbl-item>
								    ...
								    <td >'.$a++.'</td>		
									<td class=nolkpp >OFF-'.$row['idorderqc_off'].'</td>
									<td class=nopo>'.$row['nopo_off'].'</td>
									<td class=nmprd>'.$row['nam_prod'].'</td>	
									<td class=dsb >'.$row['pabrik'].'</td>									
                                    <td class=noaks>'.$row['pemesan_off'].'</td>	
									<td class=tglterima>'.$row['tglterima_off'].'</td>
									<td class=tglserah>'.$row['tglserah_off'].'</td>
									<td>'.$row['ketqc_off'].'</td>	'; 
							?>
									<td><a href="?hlm=qcoff&aksi=edit&idorderqc_off=<?=$row["idorderqc_off"]; ?>" class="cus-application_form_edit"></a>		
									<a href="#" type="button" class="cus-cross" data-toggle="modal" data-target="#myModal<?=$row["idorderqc_off"]; ?>"></a> </td>
								</tr>
								
								
			 <div class="modal fade" id="myModal<?php echo $row["idorderqc_off"]; ?>" role="dialog"  tabindex="-1" >
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form role="form" action="qcoff\hapus.php" method="get">

                        <?php
                        $idorderqc_off = $row["idorderqc_off"]; 
                        $query_edit = mysqli_query($con,"SELECT * FROM qc_off WHERE idorderqc_off='$idorderqc_off'");
                        //$result = mysqli_query($conn, $query);
                        while ($rows = mysqli_fetch_array($query_edit)) {  
                        ?>

                        <input type="hidden" name="idorderqc_off" value="<?php echo $rows["idorderqc_off"] ?>" >
						<input type="hidden" name="username" value="<?php echo $_SESSION['username'] ?>" >

                      <div class="form-group">
						<label for="message-text" class="col-form-label">Alasan ID <b>OFF-<?php echo $rows["idorderqc_off"] ?></b> dihapus ?</label>
