...
$data = mysqli_query($con,"SELECT * FROM admjual_spb INNER JOIN spb ON spb.nospb=admjual_spb.noadm_spb 
													 INNER JOIN produk_master ON produk_master.id_prod=admjual_spb.idprodadm_spb WHERE noadm_spb='$noadm_spb' ");

while ($spbadm = mysqli_fetch_array($data)) {
...
<legend class="legend_form">Info Produk</legend>
    ...
    <div class="col-sm-3">
    <input class="form-control" id="nomorspbadm" rows="2" value= "<?= $spbadm["noadm_spb"]; ?>" readonly>
    </div>
	</div>
  
	<input type="hidden" class="form-control" id="idprodadm_spb" value= "<?= $spbadm["idprodadm_spb"]; ?>" placeholder="Nama Distributor"readonly>
  
    <div class="form-group row">
    <label  class="col-sm-3 col-form-label">Nama Produk </label>
    <div class="col-sm-8">
    <textarea class="form-control" rows="2" placeholder="Nama Produk" id="nmprd" readonly><?= $spbadm["nam_prod"]; ?></textarea>
    </div>
    </div>
    ...
    <div class="col-sm-8">
    <textarea class="form-control" rows="1" placeholder="Pelanggan" id="pelanggan"  readonly><?= $spbadm["pelanggan_spb"]; ?></textarea>
    </div>
    </div>
    ...
    <div class="col-sm-8">
    <textarea class="form-control" rows="1" placeholder="Untuk" id="untuk"  readonly><?= $spbadm["untuk_spb"]; ?></textarea>
    </div>
    </div>
    ...
    <input class="form-control"  type="text"  style="height:28px; width:70px;" value= "<?= $spbadm["jumlah_spb"]; ?>" id="jumlah" placeholder="99" readonly>
	<input id="satuan_prod" class="input-group-text " readonly style="height:28px; width:90px; text-align:left;" value="<?= $spbadm["satuan_prod"]; ?>">
...
<legend class="legend_form">PO ( Purchasing Order )</legend>
    ...
    <div class="col-sm-6">
    <input type="text" class="form-control" value= "<?= $spbadm["nopo_spb"]; ?>" id="nopo_spb" placeholder="XXXX/XX/XXXX/2019">
    </div>
	</div>
    ...
    <div class="col-sm-6">
	<input type="date" class="form-control" value= "<?= $spbadm["tglpo_spb"]; ?>" id="tglpo_spb">
    </div>
	</div> 
</fieldset>
...
<legend class="legend_form">DO ( Delivery Order ) / SPK</legend>
    ...
    <div class="col-sm-6">
	<input type="text" class="form-control" id="nodo_spb" value= "<?= $spbadm["nodo_spb"]; ?>" placeholder="XXXX/XX/XXXX/2019">
    </div>
	</div>
    ...
    <div class="col-sm-6">
	<input type="date" class="form-control" value= "<?= $spbadm["tgldo_spb"]; ?>" id="tgldo_spb">
    </div>
	</div> 
...
<legend class="legend_form">Lain Lain</legend>
    ...
    <div class="col-sm-6">
	<input type="text" class="form-control" value= "<?= $spbadm["telp_spb"]; ?>" id="telp_spb" placeholder="No Telp">
    </div>
	</div>
    ...
    <div class="col-sm-6">
	<input type="text" class="form-control" value= "<?= $spbadm["sms_spb"]; ?>" id="sms_spb" placeholder="No HP / WA">
    </div>
	</div>
	...
	<div class="col-sm-8">
	<textarea class="form-control" rows="2" id="ket_admspb" placeholder="Keterangan"><?= $spbadm["ket_admspb"]; ?></textarea>
	</div>
	</div>
...
$("#check").submit(function(e) {
	e.preventDefault();
	var nomorspbadm = $("#nomorspbadm").val();
	...
	if(nomorspbadm == "" || ket_log == "") {
		$("#error_message").show().html("Harap Mengisi, Field yang wajib di isi! (*)");
	} else {
		$("#error_message").html("").hide();
		$.ajax({
			type: "POST",
			url: "check_form/edit/admspb.php",
			data: "nomorspbadm="+nomorspbadm+"&telp_spb="+telp_spb+"&sms_spb="+sms_spb+"&nopo_spb="+nopo_spb+"&tglpo_spb="+tglpo_spb+"&nodo_spb="+nodo_spb+"&tgldo_spb="+tgldo_spb+"&ket_admspb="+ket_admspb+"&idprodadm_spb="+idprodadm_spb+"&ket_log="+ket_log+"&username="+username,
