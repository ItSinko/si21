...
<body>
<h3 style="margin-bottom: -20px;" >Tabel Detail COO(Online)</h3>					
	    ...
	    <label class="nmprd-fltr">Nama Produk :</label>  
                ...
                $query = mysqli_query($con,"SELECT nam_prod FROM detailcoo_on
													...
													INNER JOIN produk_master ON produk_master.id_prod=spa_on.idprod_on GROUP by nam_prod");
		      ...
		      <label class="aks-fltr">Nomer AKS :</label>  
                ...
                $query = mysqli_query($con,"SELECT noaks_on FROM detailcoo_on
													INNER JOIN spa_on ON detailcoo_on.nolkppcoo_on=spa_on.nolkpp_on
													INNER JOIN distributor ON distributor.iddsb=spa_on.pabrik_on
													INNER JOIN produk_master ON produk_master.id_prod=spa_on.idprod_on group by noaks_on");		
                while ($row = mysqli_fetch_array($query)) {
                ?>
                ...
                $query = mysqli_query($con,"SELECT pabrik FROM detailcoo_on
													INNER JOIN spa_on ON detailcoo_on.nolkppcoo_on=spa_on.nolkpp_on
													INNER JOIN distributor ON distributor.iddsb=spa_on.pabrik_on
													INNER JOIN produk_master ON produk_master.id_prod=spa_on.idprod_on GROUP by pabrik");
                while ($row = mysqli_fetch_array($query)) {
                ?>			
...
<br>
	...
	<th width=3% class="text-center">No AKS</th>
...
$result=mysqli_query($con,"SELECT * FROM detailcoo_on
		 ...
		 data-noaks="'.$row['noaks_on'].'" >
			...
			<td>'.$a++.'</td>	
			<td>'. $row["noaks_on"].'<br><a href="detail.php?nolkppcoo_on='.$row["nolkppcoo_on"].'" class="cus-arrow_right"></a></td>	
...
$(".tes").floatThead({scrollingTop:1});
	...
	<button class="tambah"  onclick="location.href='add.php'" type="tambah" target=""  title="Menambahakan Data Ke Database Detail Coo">
	<i class="cus-add"></i> Tambah</button> 
</body>
</html>
