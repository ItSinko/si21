...
.ab:hover { background: #0000; color: #000; }  
...
<tr>
   ...
   <td class="ab" colspan="1">
    <div class="form-group">
    <label>Distributor</label>
    <input  list="daftar-dsbr"  id="dsbr-filter"  class="form-control mr-sm-2" type="search" placeholder="Distributor"  title="Filter Data Berdasarkan No AKS" >
    <datalist id="daftar-dsbr">
   <option value="Semua"></option>
                            <?php
				$con= mysqli_connect("localhost","root","","kontrol_db");
                $query = mysqli_query($con,"SELECT pabrik FROM ekspedisi_on 
														...
														INNER JOIN distributor ON ekspedisi_on.pabrikeks_on = distributor.iddsb GROUP by pabrik");
...
<input  list="daftar-nmprd"  id="nmprd-filter"  class="form-control mr-sm-2" type="search" placeholder="Type Produk "  title="Filter Data Berdasarkan Distributor" >
<datalist id="daftar-nmprd">
					<option value="Semua"></option>
                            <?php
			
                $query = mysqli_query($con,"SELECT sing_prod FROM ekspedisi_on 
														INNER JOIN gudang_on ON ekspedisi_on.nolkppeks_on = gudang_on.nolkppgdg_on
														INNER JOIN admjual_on ON ekspedisi_on.nolkppeks_on = admjual_on.nolkppadm_on
														INNER JOIN spa_on ON ekspedisi_on.nolkppeks_on = spa_on.nolkpp_on
														INNER JOIN produk_master ON ekspedisi_on.idprodeks_on = produk_master.id_prod
														INNER JOIN distributor ON ekspedisi_on.pabrikeks_on = distributor.iddsb GROUP by sing_prod");
                while ($row = mysqli_fetch_array($query)) {
                ?>
                    <option value="<?php echo $row['sing_prod']; ?>"></option>
                <?php
                }
                ?>
              </datalist>	 
 ...
 <div class="form-group">
 <label >No PO</label>
<input  list="daftar-nopo"  id="nopo-filter"  class="form-control mr-sm-2" type="search" placeholder="Distributor "  title="Filter Data Berdasarkan Distributor" >
<datalist id="daftar-nopo">
				<option value="Semua"></option>
                            <?php
			
                $query = mysqli_query($con,"SELECT nopo_on FROM ekspedisi_on 
														INNER JOIN gudang_on ON ekspedisi_on.nolkppeks_on = gudang_on.nolkppgdg_on
														INNER JOIN admjual_on ON ekspedisi_on.nolkppeks_on = admjual_on.nolkppadm_on
														INNER JOIN spa_on ON ekspedisi_on.nolkppeks_on = spa_on.nolkpp_on
														INNER JOIN produk_master ON ekspedisi_on.idprodeks_on = produk_master.id_prod
														INNER JOIN distributor ON ekspedisi_on.pabrikeks_on = distributor.iddsb GROUP by nopo_on");
                while ($row = mysqli_fetch_array($query)) {
                ?>
 ...
 <div class="form-group">
 <label >No SJ</label>
<input  list="daftar-nosj"  id="nosj-filter"  class="form-control mr-sm-2" type="search" placeholder="No SJ "  title="Filter Data Berdasarkan Distributor" >
<datalist id="daftar-nosj">
				<option value="Semua"></option>
                <?php
				
                $query = mysqli_query($con,"SELECT nosj_on FROM ekspedisi_on 
														INNER JOIN gudang_on ON ekspedisi_on.nolkppeks_on = gudang_on.nolkppgdg_on
														INNER JOIN admjual_on ON ekspedisi_on.nolkppeks_on = admjual_on.nolkppadm_on
														INNER JOIN spa_on ON ekspedisi_on.nolkppeks_on = spa_on.nolkpp_on
														INNER JOIN produk_master ON ekspedisi_on.idprodeks_on = produk_master.id_prod
														INNER JOIN distributor ON ekspedisi_on.pabrikeks_on = distributor.iddsb GROUP by nosj_on" );
                while ($row = mysqli_fetch_array($query)) {
                ?>
				
                    <option value="<?php echo $row['nosj_on']; ?>"></option>
                <?php
                }
                ?>
              </datalist>	 
...
$result=mysqli_query($con,"SELECT * FROM ekspedisi_on 
		...
		data-nopo="'.$row['nopo_on'].'"
			data-nosj="'.$row['nosj_on'].'">
		<td>'.$a++.'</td>	
		<td>'. $row["nopo_on"].'</td>	
		<td>'. $row["nosj_on"].'<br><a  href="?hlm=ekson&aksi=kirim&nolkppeks_on='.$row["nolkppeks_on"].'" class="cus-email"" title="Kirim Data"></a></td>	
		<td>'. $row["nolkppeks_on"].'</td>	
		<td class=kiri>'. $row["pabrik"].'</td>	
		<td class=kiri>'.$row["nam_prod"].'</td>	
		<td>'. $row["jumlah_on"].' Unit</td>	
		<td>'.$row["keteks_on"].'</td>			
		<td><a href="ubah.php?nolkppeks_on='.$row['nolkppeks_on'].'" class="cus-application_form_edit" title="Ubah Data"></a> | <a href="hapus.php?nolkppeks_on='.$row['nolkppeks_on'].'" class="cus-cross"" title="Hapus Data" onclick="return confirm(\'Yakin Hapus Data ?\');"></a></td>
