...
if( isset( $_REQUEST['aksi'] )){
		...
		switch($aksi){
			case 'baru':
				include 'ekspedisioff/tambah.php';
				break;
			case 'edit':
				include 'ekspedisioff/ubah.php';
				break;
			case 'more':
				include 'ekspedisioff/more.php';
				break;
			case 'delete':
				include 'ekspedisioff/hapus.php';
				break;
		}
	} else {
...
?>
	...
	<p class="h5">Tabel Ekspedisi (Offline)</p>
	<hr class="my-2">
					...
					<div class="jplist-panel box panel-top">						
					 ...
					 <!-- filter by TglPO -->
                        ...
                        <div 
						   ...
						   <button class="btn btn-primary"   onclick="location.href='./index.php?hlm=eksoff&aksi=baru'">Tambah Data</button>
					</div>		
 <!-- data --> 
                    <div class="horizon_disable">
                        <table class="demo-tbl">
							...
							<?php 
$a= 1;
$result=mysqli_query($con, "SELECT * FROM ekspedisi_off 
														...
														INNER JOIN distributor ON ekspedisi_off.pabrikeks_off = distributor.iddsb
														ORDER BY no_off DESC");		
		 ...
		 while($row = mysqli_fetch_array($result)){
                            ...
                            <tbody>                      
                                <tr class=tbl-item>
									...
									<td class=nosj>'.$row['nosj_off'].'<br> <a href="?hlm=eksoff&aksi=more&idordereks_off='.$row["idordereks_off"].'" class="cus-email"></a> </td>
									<td class=nolkpp>'.$row['idordereks_off'].'</td>	
									<td class=dsb >'.$row['pabrik'].'</td>									
                                    <td class=nmprd>'.$row['nam_prod'].'</td>	
									<td>'.$row['jumlah_off'].' Unit</td>
									<td>'.$row['keteks_off'].'</td>';	   									
								?>   	
								    <td><a href="?hlm=eksoff&aksi=edit&idordereks_off=<?=$row["idordereks_off"]; ?>" class="cus-application_form_edit"></a> |
									<a href="?hlm=eksoff&aksi=delete&idordereks_off=<?=$row["idordereks_off"]; ?>
