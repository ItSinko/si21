...
//1.mengambil inputan dari tabel penjualan	
	$con= mysqli_connect("localhost","root","","kontrol_db");
			...
			$sql1 = mysqli_query($con, "SELECT jumlah_off FROM spa_off 
							WHERE idorder_off ='$idordereks_fk' ");
...
//2. menghitung jumlah barang yang sudah diinput 			
				$sql = mysqli_query($con, "SELECT COALESCE(SUM(jumlaheks_off),0) as yo FROM ekspedisi2_off 
							WHERE idordereks_fk ='$idordereks_fk' ");
...
$mysql2= "UPDATE ekspedisi2_off SET 
			idordereks_fk = '$idordereks_fk',
			tglkirim_off= '$tglreal',
			jumlaheks_off = '$jumlah',
			koli_off = '$koli_off',
			noresi_off = '$noresi',
			nilai_off = '$nilaiharga',
			jasafk_off = '$jasafk_off',
					keteks2_off = '$keteks2_off'
			WHERE ideks_off = '$id' ";
...
echo " 
	...
	$ongkir= "UPDATE ongkirdb_off SET   nilai='$nilaiharga',idorderfk_off='$idordereks_fk' WHERE idongkir_off = '$idongkir'";
