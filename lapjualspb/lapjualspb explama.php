...
<body>
<h3 style="margin-bottom: -20px;">Laporan Penjualan (Offline)</h3>					
...
<button class="btn btn-primary" type="export" id="tombolExport"><i class="cus-arrow_down"></i>Export</button>
	    ...
	    <label class="nmprd-fltr">Nama Produk :</label>  
                ...
                $query = mysqli_query($con,"SELECT nam_prod FROM spb 
											 ...
											 INNER JOIN jasaeks ON jasaeks.id_eks = ekspedisi2_spb.jasafk_spb GROUP by nam_prod");
		      ...
		      <label class="sj-fltr">No SJ :</label>  
            	<input list="daftar-sj"  id="sj-filter" type="search" title="Filter Data Berdasarkan Nomer AKS">
				<datalist id="daftar-sj">
					<option value="Semua"></option>
                            <?php
			
                $query = mysqli_query($con,"SELECT nosjgdg_spb FROM spb 
											 INNER JOIN  admjual_spb ON admjual_spb.noadm_spb=spb.nospb
											 INNER JOIN  qc_spb ON admjual_spb.noadm_spb = qc_spb.noqc_spb
											 INNER JOIN gudang_spb ON qc_spb.noqc_spb = gudang_spb.nogdg_spb
											 INNER JOIN produk_master ON produk_master.id_prod = spb.idprod_spb
											 ...
											 LEFT JOIN  ekspedisi_spb ON seri_spb.nogdgfk = ekspedisi_spb.noeks_spb 
											 INNER JOIN ekspedisi2_spb ON ekspedisi2_spb.noeksfk_spb = spb.nospb
											 INNER JOIN jasaeks ON jasaeks.id_eks = ekspedisi2_spb.jasafk_spb GROUP by nosjgdg_spb");		
                while ($row = mysqli_fetch_array($query)) {
                ?>
				
                    <option value="<?php echo $row['nosjgdg_spb']; ?>"></option>
                ...
                ?>
			     ...
			     <label class="plg-fltr">Pelanggan :</label>  
			    <input list="daftar-plg"  id="plg-filter" type="search"  title="Filter Data Berdasarkan Distributor">
				<datalist id="daftar-plg">
				
				<option value="Semua"></option>
                <?php

                $query = mysqli_query($con,"SELECT pelanggan_spb FROM spb 
											 INNER JOIN  admjual_spb ON admjual_spb.noadm_spb=spb.nospb
											 INNER JOIN  qc_spb ON admjual_spb.noadm_spb = qc_spb.noqc_spb
											 INNER JOIN gudang_spb ON qc_spb.noqc_spb = gudang_spb.nogdg_spb
											 INNER JOIN produk_master ON produk_master.id_prod = spb.idprod_spb
											 ...
											 LEFT JOIN  ekspedisi_spb ON seri_spb.nogdgfk = ekspedisi_spb.noeks_spb 
											 INNER JOIN ekspedisi2_spb ON ekspedisi2_spb.noeksfk_spb = spb.nospb
											 INNER JOIN jasaeks ON jasaeks.id_eks = ekspedisi2_spb.jasafk_spb GROUP by pelanggan_spb");
                while ($row = mysqli_fetch_array($query)) {
                ?>			
                    <option value="<?php echo $row['pelanggan_spb']; ?>"></option>
                <?php
                }
                ?>
              </datalist>
			  ...
			  <datalist id="daftar-noseri">
				...
				<option value="Semua"></option>
                <?php
                $query = mysqli_query($con,"SELECT noseri_spb FROM spb 
											 INNER JOIN  admjual_spb ON admjual_spb.noadm_spb=spb.nospb
											 INNER JOIN  qc_spb ON admjual_spb.noadm_spb = qc_spb.noqc_spb
											 INNER JOIN gudang_spb ON qc_spb.noqc_spb = gudang_spb.nogdg_spb
											 INNER JOIN produk_master ON produk_master.id_prod = spb.idprod_spb
											 ...
											 LEFT JOIN  ekspedisi_spb ON seri_spb.nogdgfk = ekspedisi_spb.noeks_spb 
											 INNER JOIN ekspedisi2_spb ON ekspedisi2_spb.noeksfk_spb = spb.nospb
											 INNER JOIN jasaeks ON jasaeks.id_eks = ekspedisi2_spb.jasafk_spb GROUP by noseri_spb" );
                while ($row = mysqli_fetch_array($query)) {
                ?>			
                    <option value="<?php echo $row['noseri_spb']; ?>"></option>
                <?php
                }
                ?>
              </datalist>
...
<br>
	...
	<th width=1% bgcolor="#ffe0ad">Untuk</th>
	<th width=1% bgcolor="#ffe0ad">Jumlah</th>
    <th width=2% bgcolor="#ffe0ad">Alias</th>
	<th width=3% bgcolor="#ffe0ad">Nama Produk</th>
	<th width=1% bgcolor="#ffe0ad">No Seri</th>
	<th width=2% bgcolor="#ffe0ad">Harga</th>
	<th width=2% bgcolor="#ffe0ad">Total Harga</th>
	<th width=2% bgcolor="#ffe0ad">No PO</th>
	<th width=1% bgcolor="#ffe0ad">Tgl Order</th>
	<th width=1% bgcolor="#ffe0ad">Tgl Terima</th>
	<th width=1% bgcolor="#ffe0ad">Tgl Serah</th>
	<th width=2% bgcolor="#ffe0ad">No SJ</th>
	<th width=1% bgcolor="#ffe0ad">Tgl SJ</th>
	<th width=2% bgcolor="#ffe0ad">Ekspedisi</th>
	<th width=2% bgcolor="#ade1ff">No Resi</th>
	<th width=1% bgcolor="#ade1ff">Tgl Kirim</th>
	<th width=1% bgcolor="#ade1ff">Biaya Kirim</th>
    </tr>
       </thead>
		<tbody>
...
$result=mysqli_query($con,"SELECT * ,harga_spb * jumlah_spb as total FROM spb 
 ...
 while($row = mysqli_fetch_array($result)){
        ...
        echo'<tr class="task-list-row"
		 ...
		 data-sj="'.$row['nosjgdg_spb'].'"
		 data-plg="'.$row['pelanggan_spb'].'"
	     data-noseri="'.$row['noseri_spb'].'" >
			...
			<td class=kanan>'.number_format($row["harga"]).'</td>
			<td class=kanan>'.number_format($row["total"]).'</td>
			<td>'. $row["nopo_spb"].'</td>';
			
			$source = $row["tglpo_spb"];
			...
			$source = $row["tglterimaqc_spb"];
								...
								if ($source=='-')  { 
									...
									$tglterima_def = date_create($source);
				...
				$source = $row["tglserahqc_spb"];
								...
								if ($source=='-')  { 
									...
									$tglserah_def = date_create($source);
			...
			echo' <td>'. $row["nosjgdg_spb"].'</td>';
							
		$source = $row["tglsjgdg_spb"];
								...
								if ($source=='-')  { 
									...
									$tglsj_def = date_create($source);

									
								  $tglsj = date_format($tglsj_def,"d/m/Y");
									echo '<td>'.$tglsj.'</td>';
												}
		...
		$source = $row["tglkirim_spb"];
			...
			echo'
			<td class=kanan>'. number_format($row["nilai_spb"]).'</td>			
		    </tr>';     
