<html>
<head>
<title>Laporan Keuangan (Online)</title>
</head>
<style>
...
<body>
<h3 style="margin-bottom: -20px;">Laporan Keuangan (Online)</h3>					
...
<button class="btn btn-primary" type="export" id="tombolExport"><i class="cus-arrow_down"></i>Export</button>
	    ...
	    <label class="nmprd-fltr">Nama Produk :</label>  
                ...
                $query = mysqli_query($con,"SELECT nam_prod FROM acc_on												
												...
												INNER JOIN piutang_on ON piutang_on.nolkppuji_on = acc_on.nolkppacc_on
		      ...
		      <label class="aks-fltr">Nomer AKS :</label>  
                ...
                $query = mysqli_query($con,"SELECT noaks_on FROM acc_on												
												INNER JOIN gudang_on ON gudang_on.nolkppgdg_on = acc_on.nolkppacc_on
												INNER JOIN spa_on ON spa_on.nolkpp_on = acc_on.nolkppacc_on
												INNER JOIN piutang_on ON piutang_on.nolkppuji_on = acc_on.nolkppacc_on
												INNER JOIN ongkirdb_on ON ongkirdb_on.nolkppkirfk_on = acc_on.nolkppacc_on
                ...
                $query = mysqli_query($con,"SELECT pabrik FROM acc_on												
												INNER JOIN gudang_on ON gudang_on.nolkppgdg_on = acc_on.nolkppacc_on
												INNER JOIN spa_on ON spa_on.nolkpp_on = acc_on.nolkppacc_on
												INNER JOIN piutang_on ON piutang_on.nolkppuji_on = acc_on.nolkppacc_on
												INNER JOIN ongkirdb_on ON ongkirdb_on.nolkppkirfk_on = acc_on.nolkppacc_on
...
$result=mysqli_query($con,"SELECT   * ,harga_on * jumlah_on as total
								   ...
								   ,((harga_on * jumlah_on) - ((harga_on * jumlah_on) * diskonacc_on / 100)) + SUM(nilai)  as totpiu
									...
									,IF(iddsb='SPA',
									  ((((harga_on * jumlah_on) - ((harga_on * jumlah_on) * diskonacc_on / 100)) + SUM(nilai))/1.1) * 0.985,
									  (harga_on * jumlah_on) - ((harga_on * jumlah_on) * diskonacc_on / 100) + SUM(nilai) ) AS nilaiterima,
									  
									  IF(iddsb='SPA',
									  (((((harga_on * jumlah_on) - ((harga_on * jumlah_on) * diskonacc_on / 100)) + SUM(nilai))/1.1) * 0.985)  - realisasiaccon,
									  ((harga_on * jumlah_on) - ((harga_on * jumlah_on) * diskonacc_on / 100) + SUM(nilai)) - realisasiaccon ) AS selisihharga
		...
		<td class=kanan>'. number_format($row["dppfaktur"]).'</td>	
		<td class=kanan>'. number_format($row["ppnfaktur"]).'</td>	
		<td class=kanan>'. number_format($row["kirim"]).'</td>
		<td class=kanan>'. number_format($row["totpiu"]).'</td>	
		<td>'. number_format($row["ppn"]).'</td>
		<td>'. number_format($row["pph"]).'</td>	
		<td>'. number_format($row["nilaiterima"]).'</td>
		<td>'. $row["temphariacc_on"].' Hari</td>
		<td>'. $bayarbatas.'</td>
		<td class=kanan>'. number_format($row["realisasiaccon"]).'</td>
		
		<td class=kanan>'.$row["tgllunas_on"].'</td>
