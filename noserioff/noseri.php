...
?>
	...
	<p class="h5">Nomer Seri (Offline)</p>
	<hr class="my-2">
...
$a= 1;
$result=mysqli_query($con,"SELECT *, if(isnull(noseri_off),'0', '1') as status_seri  FROM admjual_off INNER JOIN spa_off ON spa_off.idorder_off = admjual_off.idorderadm_off
							...
							LEFT JOIN  seri_off ON admjual_off.idorderadm_off = seri_off.idorderfk_off
							LEFT JOIN  gudang_off ON admjual_off.idorderadm_off = gudang_off.idordergdg_off
							 GROUP BY idorderadm_off
		...
		echo'                 
                            <tbody>                      
                                <tr class=tbl-item>
									...
									<td >'.$row['jumlah_off'].' '.$row['satuan_prod'].' <br> <a href="?hlm=noserioff&aksi=more&idorderadm_off='.$row["idorderadm_off"].'" class="cus-arrow_right"></a></td>	
