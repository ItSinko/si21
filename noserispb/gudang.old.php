...
<p class="h5">Tabel Gudang(Online)</p>
<hr class="my-2">
...
<tr>
   ...
   <td class="ab" colspan="1">
    ...
    <input  list="daftar-nmprd"  id="nmprd-filter"  class="form-control mr-sm-2" type="search" placeholder="Type Produk "  title="Filter Data Berdasarkan No AKS" >
    <datalist id="daftar-nmprd">
    <option value="Semua"></option>
                            <?php
				$con= mysqli_connect("localhost","root","","kontrol_db");
                $query = mysqli_query($con,"SELECT sing_prod FROM gudang_on INNER JOIN spa_on
				 ON gudang_on.nolkppgdg_on=spa_on.nolkpp_on
				 INNER JOIN produk_master ON produk_master.id_prod = gudang_on.idprodgdg_on
				 INNER JOIN distributor ON distributor.iddsb=gudang_on.pabrikgdg_on GROUP by sing_prod");
 ...
 <div class="form-group">
 <label >No AKS</label>
<input  list="daftar-noaks"  id="noaks-filter"  class="form-control mr-sm-2" type="search" placeholder="No AKS "  title="Filter Data Berdasarkan Distributor" >
<datalist id="daftar-noaks">
					<option value="Semua"></option>
                            <?php

                $query = mysqli_query($con,"SELECT noaks_on FROM gudang_on INNER JOIN spa_on
				 ON gudang_on.nolkppgdg_on=spa_on.nolkpp_on
				 INNER JOIN produk_master ON produk_master.id_prod = gudang_on.idprodgdg_on
				 INNER JOIN distributor ON distributor.iddsb=gudang_on.pabrikgdg_on GROUP by noaks_on");		
                while ($row = mysqli_fetch_array($query)) {
                ?>
...
<input  list="daftar-dsb"  id="dsb-filter"  class="form-control mr-sm-2" type="search" placeholder="Distributor "  title="Filter Data Berdasarkan Distributor" >
<datalist id="daftar-dsb">
					<option value="Semua"></option>
                <?php

                $query = mysqli_query($con,"SELECT pabrik FROM gudang_on INNER JOIN spa_on
				 ON gudang_on.nolkppgdg_on=spa_on.nolkpp_on
				 INNER JOIN produk_master ON produk_master.id_prod = gudang_on.idprodgdg_on
				 INNER JOIN distributor ON distributor.iddsb=gudang_on.pabrikgdg_on GROUP by pabrik");
                while ($row = mysqli_fetch_array($query)) {
                ?>			
...
<br>
	...
	<th width=4% class="text-center">No LKPP</th>
	<th width=4% class="text-center">No SJ</th>
	<th width=4% class="text-center">Jumlah</th>
	<th width=4% class="text-center">Nama Produk</th>
	<th width=4% class="text-center">Tgl SJ</th>
...
$result=mysqli_query($con,"SELECT * FROM gudang_on INNER JOIN spa_on
		...
		>
			...
			<td>'.$a++.'</td>	
			<td>'. $row["nolkppgdg_on"].'<br><a href="more.php?nolkppgdg_on='.$row["nolkppgdg_on"].'" class="cus-arrow_right"></a></td>	
			<td>'. $row["nosj_on"].'</td>
	...
	echo '<td>'.$tglsj.'</td>';
			...
			echo'
			<td>'. $row["ketgdg_on"].'</td>
			<td><a href="change.php?nolkppgdg_on='.$row['nolkppgdg_on'].'" class="cus-application_form_edit" title="Ubah Data"></a> | <a href="hapus.php?nolkppgdg_on='.$row['nolkppgdg_on'].'"  class="cus-cross"" title="Hapus Data" onclick="return confirm(\'Yakin Hapus Data ?\');"></a></td>
		</tr>';     
