...
<?php
 ...
 header("content-disposition: attachment;filename=Akunting(SPB).xls");
...
<br>
	...
	<th width=6% class="text-center">No Fakju</th>
	<th width=10%>Nama Produk</th>
    <th width=7%>Pelanggan</th>
	<th width=8%>Tgl SJ</th>
	<th width=4%>Jumlah</th>
	<th width=5%>Harga Produk</th>
	<th width=4%>Total</th>
	<th width=5%>Diskon FP</th>
	<th width=5%>Total FP</th>
	<th width=5%>Biaya Ekspedisi</th>
	<th width=5%>Total Piutang</th>
	<th width=5%>Tgl Lunas</th>
	<th width=5%>Nilai Terima</th>
	<th width=7%>Kas</th>	
...
$result=mysqli_query($con,"SELECT *,harga_spb * jumlah_spb as total 
 ...
 while($row = mysqli_fetch_array($result)){
     ...
     //batas	
			...
			<td class=kanan>'. number_format($row["harga_spb"]).'</td>
			<td class=kanan>'. number_format($row["total"]).'</td>
			<td class=kanan>'. number_format($row["diskonfp_spb"]).'</td>
			<td class=kanan>'. number_format($row["total_fp"]).'</td>
			<td class=kanan>'. number_format($row["kirim"]).'</td>
			<td class=kanan>'. number_format($row["totpiu"]).'</td>';
			
			//batas
				$sources = $row["tgllunas_spb"];
						if ($sources=='-')  { 
									echo '<td>-</td>';
									...
									$tgllunas_def = date_create($sources);

									
								  $tgllunas = date_format($tgllunas_def,"d/m/Y");
									echo '<td>'.$tgllunas.'</td>';
												}	
