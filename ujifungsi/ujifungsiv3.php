...
<h3 style="margin-bottom: -20px;">Piutang Usaha (Online)</h3>					
	...
	<th width=20%>Tombol</th>
			...
			$query=mysqli_query($con,"SELECT * FROM ujifungsi INNER JOIN accspa ON accspa.nolkppacc=ujifungsi.lkppuji");	
...
<td class="nmprd"><?= $row["namaprodukacc"]; ?></td>
<td ><?= $row["jumlahprdacc"]; ?> Pcs</td>
<td><?= number_format($row["totalacc"]); ?></td>
